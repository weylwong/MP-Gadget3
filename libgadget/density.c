...
density(int update_hsml, int DoEgyDensity, ForceTree * tree)
{
    ...
    if(All.BlackHoleOn && update_hsml) {
...
void density_check_neighbours_int (int i, TreeWalk * tw, MyFloat MaxHsml, MyFloat DensFac) {
    ...
    if(BHDENSITY_GET_PRIV(tw)->NIteration >= MAXITER - 10)
    {
         message(1, "i=%d task=%d ID=%lu Hsml=%g Left=%g Right=%g Ngbs=%g Right-Left=%g\n   pos=(%g|%g|%g)\n",
             i, ThisTask, P[i].ID, P[i].Hsml, Left[PI], Right[PI],
             (float) P[i].NumNgb, Right[PI] - Left[PI], P[i].Pos[0], P[i].Pos[1], P[i].Pos[2]);
    }
