...
void test_drift_factor(void ** state)
{
    ...
    OmegaM = 1.;
    update_drift_factor(get_ti(0.8), get_ti(0.85), 1);
    ...
    /*Test the kick table*/
    update_kick_factors(get_ti(0.8), get_ti(0.8), 1);
    assert_true(fabs(get_gravkick_factor(get_ti(0.8), get_ti(0.85), -1) - 2/0.1*(sqrt(0.85) - sqrt(0.8))) < 5e-5);
    /*Test the hydrokick table: always the same as drift*/
    assert_true(fabs(get_hydrokick_factor(get_ti(0.8), get_ti(0.85), -1) - get_drift_factor(get_ti(0.8), get_ti(0.85))) < 5e-5);

    //Now choose a more realistic cosmology
    OmegaM = 0.25;
    /*Check late and early times*/
    update_drift_factor(get_ti(0.95), get_ti(0.98), 1);
    assert_true(fabs(get_drift_factor(get_ti(0.95), get_ti(0.98)) - exact_drift_factor(0.95, 0.98,3)) < 5e-5);
    update_drift_factor(get_ti(0.05), get_ti(0.06), 1);
    assert_true(fabs(get_drift_factor(get_ti(0.05), get_ti(0.06)) - exact_drift_factor(0.05, 0.06,3)) < 5e-5);
    /*Check boundary conditions*/
    double logDtime = (log(AMAX)-log(AMIN))/(1<<TIMEBINS);

    update_drift_factor(((1<<TIMEBINS)-1), 1<<TIMEBINS, 1);
    assert_true(fabs(get_drift_factor(((1<<TIMEBINS)-1), 1<<TIMEBINS) - exact_drift_factor(AMAX-logDtime, AMAX,3)) < 5e-5);
    update_drift_factor(0, 1, 1);
    assert_true(fabs(get_drift_factor(0, 1) - exact_drift_factor(1.0 - exp(log(AMAX)-log(AMIN))/(1<<TIMEBINS), 1.0,3)) < 5e-5);
    /*Gravkick*/
    update_kick_factors(get_ti(0.8), get_ti(0.8), 1);
    assert_true(fabs(get_gravkick_factor(get_ti(0.8), get_ti(0.85), -1) - exact_drift_factor(0.8, 0.85, 2)) < 5e-5);
    update_kick_factors(get_ti(0.05), get_ti(0.05), 1);
    assert_true(fabs(get_gravkick_factor(get_ti(0.05), get_ti(0.06), -1) - exact_drift_factor(0.05, 0.06, 2)) < 5e-5);
}
