...
void displacement_fields(int Ngrid, int Type, const double * shift) {
    ...
    double maxdispall = 0, maxdisp = 0, maxvel = 0, maxvelall = 0;
    int i;
    ...
    MPI_Reduce(&maxvel, &maxvelall, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
    message(0, "Max vel=%g km/s, vel_prefac= %g  hubble_a=%g fom=%g \n", sqrt(maxvelall), vel_prefac, hubble_a, F_Omega(All.TimeIC));
